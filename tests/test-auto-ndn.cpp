...
BOOST_AUTO_TEST_SUITE(TestAutoNdn)
...
BOOST_AUTO_TEST_CASE(TestGetCipKey)
{
  ...
  for (const auto& sentInterest : face.sentInterests) {
    ...
    didMatchInterest = didMatchInterest || sentInterest == AutoNdn::CIP_KEY_INTEREST;
  }
  ...
  data->setContent(reinterpret_cast<const uint8_t*>(REPLY_CONTENT), REPLY_CONTENT.size());
